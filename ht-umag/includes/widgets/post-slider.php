<?php
/**
 * Highthemes Post Slider
 */

class ht_post_slider extends WP_Widget {
	public function __construct() {
		$widget_ops = array(
			'classname'   => 'ht_post_slider',
			'description' => __( 'Display Posts With Slider', 'highthemes' ),
			'panels_groups' => array( 'highthemes' )

		);
		parent::__construct(
			'ht_post_slider',
			'Highthemes - ' . __( 'Post Slider', 'highthemes' ),
			$widget_ops // Args
		);
	}
	...
	public function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Featured Posts','highthemes') : $instance['title'], $instance, $this->id_base);
	...
	?>
		<div>
			<div class="picture">
				<div class="category-image">
					...
					if( has_post_thumbnail() ) {
						the_post_thumbnail('post_thumbnail_b');
					} else {
						echo '<img src="'. esc_url(get_template_directory_uri() . '/images/grey_img.jpg') .'" alt="" >';
					}
					?>
					<div class="bg">&nbsp;</div>
					<?php
					if( $category[0] ){
						...
						echo '<h2 class="overlay-category">'. $cat_name .'</h2>';
					}
					?>
			...
			<div class="detail">
				<h3 class="caption"><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a></h3>
				<div class="info">
